...
namespace PathCreationEditor {
    ...
    //                        var currentPosition = creator.bezierPath.GetPoint(handleIndexToDisplayAsTransform);
    ...
    //                    int.TryParse(nodeNetCreator.gameObject.name.Substring(5), out int id);
    ...
    //            float handleRadius = GetHandleDiameter (globalDisplaySettings.anchorSize * data.bezierHandleScale, bezierPath.GetPoint(handleIndex)) / 2f;
    //            float dst = HandleUtility.DistanceToCircle (bezierPath.GetPoint(handleIndex), handleRadius);
    //            if (dst == 0) {
    ...
    //                    float dstCamToEndpoint = (Camera.current.transform.position - bezierPath.GetPoint(bezierPath.NumPoints - 1)).magnitude;
    ...
    //        foreach (PathCreator path in nodeNetCreator.childPaths)
    //        {
    //            BezierPath bezierPath = path.bezierPath;
    ...
    //                {
    //                    DrawHandle(i, bezierPath);
    //                    DrawHandle(i + 1, bezierPath);
    //                }
    //            }
    //        }        
    //    }

    //    void DrawHandle (int i, BezierPath bezierPath) {
    //        Vector3 handlePosition = bezierPath.GetPoint(i);

    //        float anchorHandleSize = GetHandleDiameter (globalDisplaySettings.anchorSize * data.bezierHandleScale, bezierPath.GetPoint(i));
    //        float controlHandleSize = GetHandleDiameter (globalDisplaySettings.controlSize * data.bezierHandleScale, bezierPath.GetPoint(i));
    ...
    //                Vector3 dir = (bezierPath.GetPoint(attachedControlIndex) - handlePosition).normalized;
    ...
    //        if (bezierPath.GetPoint(i) != handlePosition) {
    //            Undo.RecordObject (creator, "Move point");
    ...
    //                        var anchorPoint = bezierPath.GetPoint(anchorIndex);
    ...
    //        nodeNetCreator = creator.NetCreator;
    //        bool in2DEditorMode = EditorSettings.defaultBehaviorMode == EditorBehaviorMode.Mode2D;
