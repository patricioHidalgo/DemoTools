...
namespace PathCreation.Utility
{
  ...
  public static class CubicBezierUtility
  {
    ...
    private static float EvaluateDerivativeOf(float a1, float c1, float c2, float a2, float t)
    {
      float s1 = 3 * (1 - t) * (1 - t);
      float s2 = 6 * (1 - t) * t;
      float s3 = 3 * t * t;

      return s1 * (c1 - a1) + s2 * (c2 - c1) + s3 * (a2 - c2);
    }
    ...
    private static float EvaluateSecondDerivativeOf(float a1, float c1, float c2, float a2, float t)
    {
      t = t < 0f ? 0f : t > 1f ? 1f : t;
      ...
      return s1 * (c2 - 2 * c1 + a1) + s2 * (a2 - 2 * c2 + c1);
    }

    private static float EvaluateCurvature(Vector3 a1, Vector3 c1, Vector3 c2, Vector3 a2, float t)
    {
      t = t < 0f ? 0f : t > 1f ? 1f : t;

      float dx = EvaluateDerivativeOf(a1.x, c1.x, c2.x, a2.x, t);
      float dy = EvaluateDerivativeOf(a1.y, c1.y, c2.y, a2.y, t);
      float dz = EvaluateDerivativeOf(a1.z, c1.z, c2.z, a2.z, t);

      float ddx = EvaluateSecondDerivativeOf(a1.x, c1.x, c2.x, a2.x, t);
      float ddy = EvaluateSecondDerivativeOf(a1.y, c1.y, c2.y, a2.y, t);
      float ddz = EvaluateSecondDerivativeOf(a1.z, c1.z, c2.z, a2.z, t);

      float n = Mathf.Sqrt(
                  Mathf.Pow((ddz * dy - ddy * dz), 2) +
                  Mathf.Pow((ddx * dz - ddz * dx), 2) +
                  Mathf.Pow((ddy * dx - ddx * dy), 2)
                          );

      float d = Mathf.Pow(Mathf.Pow(dx, 2) + Mathf.Pow(dy, 2) + Mathf.Pow(dz, 2), 3 / 2);

      return n / d;
    }

    public static float EvaluateCurvature(Vector3[] points, float t)
    {
      if (points.Length == 4)
        return EvaluateCurvature(points[0], points[1], points[2], points[3], t);
      Debug.Assert(true);
      return -1;
    }
    ...
    {
      if (points.Length == 4)
        return EstimateCurveLength(points[0], points[1], points[2], points[3]);
      Debug.Assert(true);
      return -1;
    }
    ...
    /// <summary>
    /// Very rough aproximation
    /// </summary>
    /// <returns></returns>
    public static float EstimateDistanceToCurve(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 worldPos)
    {
      Vector3 closestAnchorAControlA  = MathUtility.ClosestPointOnLineSegment(worldPos, p0, p1);
      Vector3 closestControlAControlB = MathUtility.ClosestPointOnLineSegment(worldPos, p1, p2);
      Vector3 closestControlBAnchorB  = MathUtility.ClosestPointOnLineSegment(worldPos, p2, p3);

      float dstAnchorAControlA  = Vector3.Distance(worldPos, closestAnchorAControlA);
      float dstControlAControlB = Vector3.Distance(worldPos, closestControlAControlB);
      float dstControlBAnchorB =  Vector3.Distance(worldPos, closestControlBAnchorB);

      return Mathf.Min(dstAnchorAControlA, Mathf.Min(dstControlAControlB, dstControlBAnchorB));
    }

    public static float EstimateDistanceToCurve(Vector3[] points, Vector3 worldPos)
    {
      if (points.Length == 4)
        return EstimateDistanceToCurve(points[0], points[1], points[2], points[3], worldPos);
      Debug.Assert(true);
      return -1;
    }

    public static int AproximateAmountOfCurvature(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3)
    {
      float angleA = Mathf.Clamp(Vector3.Angle((p3 - p0), (p1 - p0)), 0f, 90f);
      float angleB = Mathf.Clamp(Vector3.Angle((p0 - p3), (p2 - p3)), 0f, 90f);

      float normalizedAvrg = (angleA + angleB) / 180f;

      return Mathf.RoundToInt(normalizedAvrg * 100);
    }

    public static int AproximateAmountOfCurvature(Vector3[] points)
    {
      if (points.Length == 4)
        return AproximateAmountOfCurvature(points[0], points[1], points[2], points[3]);
      Debug.Assert(true);
      return -1;
    }
  }
